...
import { LineSegmentsGeometry } from '/node_modules/three/addons/lines/LineSegmentsGeometry.js';
import { LineSegments2 } from '/node_modules/three/addons/lines/LineSegments2.js';
import { LineMaterial } from '/node_modules/three/addons/lines/LineMaterial.js';
...
const CRTShader = {
    uniforms: {
        ...
        gridScale: { value: 0.5 },         // Reduced scale for more visible grid
        backgroundColor: { value: new THREE.Vector3(0.5, 0.5, 0.5) },  // Grey background tint
    ...
    fragmentShader: `
        ...
        vec2 curveRemapUV(vec2 uv) {
            ...
            cuv *= 1.0 + barrelDistance * barrelDistance * 0.05;
        ...
        void main() {
            ...
            vec4 r = texture2D(tDiffuse, uv + vec2(chromaticOffset, 0.01));
            vec4 g = texture2D(tDiffuse, uv);
            vec4 b = texture2D(tDiffuse, uv - vec2(chromaticOffset, 0.01));
            vec4 baseColor = vec4(r.r, g.g, b.b, g.a);

            // Sample with color separation
            vec4 rColor = texture2D(tDiffuse, uv + vec2(colorBleedSpread, 0.01));
            vec4 bColor = texture2D(tDiffuse, uv - vec2(colorBleedSpread, 0.01));
            ...
            finalColor = mix(centerColor, centerColor + glow, 0.5);
            ...
            finalColor.rgb = mix(backgroundColor, finalColor.rgb, 0.5); // Reduce mix to maintain grey tint
            finalColor.rgb += backgroundColor * 0.05; // Increased ambient grey
            ...
            float edgeGlow = (1.0 - border) * 0.4;
            finalColor.rgb += vec3(0.5, 0.5, 0.5) * edgeGlow;
...
const D20 = {
    ...
    init: function (scene) {
        ...
        D20.d20 = new LineSegments2(lineGeometry, material);
...
const RendererModule = {
    ...
    init: function () {
        ...
        const renderPass = new RenderPass(RendererModule.d20Scene, RendererModule.camera);
        const pixelatePass = new ShaderPass(RendererModule.pixelateShader);
        ...
        RendererModule.d20Composer = new EffectComposer(RendererModule.renderer);
